...
/** Flags a missing healthIssueDetails when "OTHER" is one of the selected healthIssues. */
const requireHealthIssueDetails = (data: { healthIssues: string[]; healthIssueDetails?: string }, ctx: z.RefinementCtx) => {
...
export const studentBaseSchema = studentObjectSchema.superRefine(requireHealthIssueDetails);
...
/**
 * Quick-add variant: same fields as studentBaseSchema, but joiningDate,
 * numberOfSessions, and paymentReceived are required — used by the admin's
 * fast "just the essentials" add-student form. Everything else (DOB, gender,
 * mobile, health info, photo) is left for the student to fill in later via
 * the self-edit link.
 */
export const quickAddStudentSchema = studentObjectSchema
  .extend({
    joiningDate: z.coerce.date({ error: "Enter a valid date of joining" }),
    numberOfSessions: z.coerce
      .number({ error: "Enter a valid number of sessions" })
      .int("Sessions must be a whole number")
      .positive("Number of sessions must be greater than 0")
      .max(1000, "Enter a realistic number of sessions"),
    paymentReceived: z.coerce
      .number({ error: "Enter a valid amount" })
      .positive("Payment received must be greater than ₹0")
      .max(10_000_000, "Enter a realistic payment amount"),
  })
  .superRefine(requireHealthIssueDetails);

export type QuickAddStudentInput = z.input<typeof quickAddStudentSchema>;
export type QuickAddStudentValues = z.output<typeof quickAddStudentSchema>;
