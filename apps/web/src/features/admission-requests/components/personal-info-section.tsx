...
import { GENDER_OPTIONS } from "@/features/student-admission/constants";
import { DocumentUpload } from "./document-upload";
import type { ApplyUseFormReturn } from "../client-schema";

export function PersonalInfoSection({ form }: { form: ApplyUseFormReturn }) {
  const {
    register,
    control,
    watch,
  ...
  return (
    <FormSection icon={User} title="Personal Information">
      <FieldWrapper label="Your photo" hint="Optional" error={errors.photo?.message}>
        <Controller
          name="photo"
          control={control}
          render={({ field }) => <PhotoUpload value={field.value} onChange={field.onChange} error={errors.photo?.message} />}
        />
      </FieldWrapper>

      <FieldWrapper label="Aadhar card" hint="Optional" error={errors.aadhar?.message}>
        <Controller
          name="aadhar"
          control={control}
          render={({ field }) => <DocumentUpload value={field.value} onChange={field.onChange} error={errors.aadhar?.message} />}
        />
      </FieldWrapper>
